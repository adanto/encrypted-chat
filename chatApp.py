...
# Here we are listening to the clients. If we receive a accept in the server, it starts a thread that will read the message
...
# Tries to connect to another client using a socket connecting
...
# for every client in our list, we start a thread with this method that reads the response and writes it
...
# For every client, we send a message to them
